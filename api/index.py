...
async def upload_ai_report(
    asset_id: int = Form(...), description: str = Form(...), 
    ...
    if not target_asset: raise HTTPException(status_code=404)
    ...
    # Impact Asset
    target_asset["health_score"] = max(0.0, target_asset["health_score"] - (ai_severity * 1.5))
    if target_asset["health_score"] < 40: target_asset["maintenance_priority"] = "Emergency"
...
def trigger_flood_alert():
    ...
    for asset in assets:
        if asset["asset_type"] == "Road":
            ...
            if asset["health_score"] < 50: asset["maintenance_priority"] = "High"
            count += 1
    save_assets(assets)
...
def resolve_report(report_id: int):
    ...
    if asset:
        asset["health_score"] = min(100.0, asset["health_score"] + (report["severity"] * 1.5))
...
def setup_demo():
    ...
    for i, data in enumerate(demo_assets):
        ...
        if data["name"] == "Napier Bridge": h_score = 92.5
        elif data["asset_type"] == "Road": h_score = max(15.0, 100.0 - (age * 3.0))
        else: h_score = max(20.0, 100.0 - (age * 0.5))
